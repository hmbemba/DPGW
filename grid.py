...
if __name__ == '__main__':
    ...
    with dpg.window(no_scrollbar=True, no_background=True) as win:
        grid = Grid(win, cols=6, rows=6, padding=(0, 0), spacing=(0, 0))

        # Without additional arguments, items will expand and shrink to the cell's size.
        grid.pack(create_button(),  0,  0, 4,2 )  # first row
        grid.pack(create_button(), 5,  0,5,5  )  # last row
        ...
        # grid.pack(create_button(), 1, 2, max_width=25, anchor="n")   # north (centered)
        # grid.pack(create_button(), 4, 4, max_height=25, anchor="sw")
